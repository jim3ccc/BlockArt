...
// Can return the following errors:
// - DisconnectedError
func OpenCanvas(minerAddr string, privKey ecdsa.PrivateKey) (canvas Canvas, setting shared.CanvasSettings, err error) {
	//dial
	//sign a token with the private key then send it... need to send the string and the two int signatures
	fmt.Println("OpenCanvas")

	client, err := rpc.DialHTTP("tcp", minerAddr)
	fmt.Println("open canvas after dial")
	if err != nil {
		return nil, shared.CanvasSettings{}, err
	}


	str := []byte("hello miner")
	r, s, err := ecdsa.Sign(rand.Reader, &privKey, str)
	if err != nil {
		fmt.Println("open canvas error")
	}
	fmt.Println("1")
	args := &shared.OpenCanvasArgs{Str: str, R: r, S: s}
	//connect
	err = client.Call("Miner.InitializeLib", args, &setting)
	if err != nil{
		fmt.Println("5", err)
	...
	canvas = &CanvasImpl{miner: client, settings: setting, serverAddr: minerAddr}
